...
export default function EnhancedCodeIDE() {
  ...
  const convertToFileSystemItem = (
    ...
    for (const [childKey, value] of Object.entries(tree)) {
      children[childKey] = convertToFileSystemItem(
  ...
  const handleFileSelect = async (file: FileSystemItem, path: string) => {
    if (file.type === "file") {
      try {
        ...
        // Remove 'root' from the beginning if it exists
        normalizedPath = normalizedPath.replace(/^\/root/, "");

        const requestUrl = `${process.env.NEXT_PUBLIC_BACKEND_URL}/files/content?path=${encodeURIComponent(normalizedPath)}`;
        console.log("Request URL:", requestUrl);

        const res = await axios.get<{ content: string }>(requestUrl);
        ...
        if (axios.isAxiosError(err)) {
          ...
          setError(
            `Failed to fetch file content: ${err.response?.data?.message || err.message}`
          );
        } else {
  ...
  useEffect(() => {
    if (!isSaved && code) {
      const timer = setTimeout(() => {
        console.log("Sending file change event", selectedFile);
        socket.emit("file:change", {
          path: selectedFile?.name ? `${currentPath}/${selectedFile.name}` : "", // Ensure correct path
          content: code,
        });
      }, 5 * 1000);
      return () => {
  ...
  const addItem = (path: string, newItem: FileSystemItem) => {
    const pathParts = path.split("/root").filter(Boolean);
    ...
    loader.init().then((monaco) => {
      monaco.editor.defineTheme("customTheme", {
        ...
        colors: {
          ...
          "editor.background": "#0F1515",
          "editor.foreground": "#0F1515",
          ...
          "editorGutter.background": "#0F1515",
      ...
      <div className="flex flex-col flex-1">
        <div className="flex-1 overflow-hidden">
          {selectedFile ? (
            ...
            <div className="flex items-center justify-center h-full bg-[#0F1515] text-gray-400">
